...
__global__ void ExecuteAES(byte operation, byte* gpuBuffer, byte* ExpandKey_gpu, int nStates, int rounds) {
	...
	int round_it, dump_it, thread_idx;
	...
	if (thread_idx < nStates) {
		...
		for (dump_it = 0; dump_it < 16; dump_it++) {
			State[dump_it / 4][dump_it % 4] = gpuBuffer[thread_idx * 16 + dump_it];
		}
...
int main(int argc, char** argv) {
	...
	if (argc == 6) {
		...
		cuda_error = cudaMemcpy(gpuExpKeyBuffer, ExpandKey, sizeof(byte) * 60 * 4, cudaMemcpyHostToDevice);
		if (cuda_error != cudaSuccess) {
		...
		while (bytesRead > 0) {
			...
			printf("Data buffer processed in %LF seconds\n", processedTime);
