/*
        ...
        DE LA TECNOLOGIA CUDA PER LвЂ™ACCELERACIo DE PROCESSOS" ("FEASIBILITY STUDY 
...
// Include libraries for multithreading
#include <pthread.h>
...
// This variable is used to get each thread id
int thread_idx = 0;
// The mutex to manage the thread index and processed bytes modification properly
pthread_mutex_t count_mutex;
// The number of bytes processed
unsigned long bytes_processed = 0L;
...
/**
 * This function is the one each thread executes.
 * 
 * Each thread who executes this will calculate its id. Starting at 0, the
 * threads will get the value form the variable and incrementing it
 * for the next thread. So first thread will get 0, the second 1, and os on.
 * 
 * After that, each thread will process the positions of the buffer according
 * to its id and the amount of threads on execution.
 * I.e.: For thread with id = 0 and 10 threads, the matrix to process will
 * be at positions 0, 10*16, 20*16, and so on.
 * 
 * @return Nothing
 */
void *threadAESExecutor(){
    // State matrix at thread level which will store the data on each AES round
    byte State[4][4];
    // The id of the thread which execute this
    int id;
    // Get the id for the current thread
    pthread_mutex_lock(&count_mutex);
    id = thread_idx++;
    pthread_mutex_unlock(&count_mutex);
    // The iterator for the state matrix
    int states_it = id;
    
    while(states_it <= nStatesInBuffer){
        ...
        states_it += THREAD_POOL_SIZE;
        
        // Update percentage
        pthread_mutex_lock(&count_mutex);
        bytes_processed += 16;
        if(states_it % 5000 == 0){
            printf("Processed %lu%% from the buffer       \r", bytes_processed *100 / (nStatesInBuffer*16));
        }
        pthread_mutex_unlock(&count_mutex);
    }
}
    ...
    if (argc == 6) {
        ...
        while (bytesRead = LoadDataBuffer(inFile)) {
            ...
            for(int thread_idx = 0; thread_idx < THREAD_POOL_SIZE; thread_idx++){
                ...
                pthread_create(&(thread_ids[thread_idx]),NULL,threadAESExecutor, &thread_idx);
            }
